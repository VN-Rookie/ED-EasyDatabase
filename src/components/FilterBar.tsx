...
export function FilterBar({ columns, columnMeta, filters, onChange, onApply, isMongo, mongoFilter = "", onMongoFilterChange, hasActiveFilter: _hasActiveFilter }: Props) {
          ...
          {hasMongoFilter && (
            <>
              <button onClick={() => mongoJsonValid && onApply()} disabled={!mongoJsonValid}
                className="text-[10px] text-emerald-400 hover:text-emerald-300 font-semibold px-2 py-1 rounded-lg hover:bg-emerald-500/10 disabled:opacity-40 transition-all shrink-0">
                Apply
              </button>
